...
RecurrenceRule.valueForCronComponent = function(component, min, max, shiftIdxs){
	...
	for (var i = 0; i < items.length; i++)
	{
		...
		{
			var itemIntVal = parseInt(item, 10);
			itemIntVal = (max == 6 && itemIntVal == 7) ? 0 : itemIntVal;
			result.push(itemIntVal - ((shiftIdxs) ? 1 : 0));
		}
		else if (item == 'l')
		{
			// TODO: last
		...
		else
		{
			...
			else
			{
				...
				else
				{
					...
					itemRange.start = (max == 6 && itemRange.start == 7) ? 0 : itemRange.start;
				}
				...
				if (rangeParts.length == 2)
				{
					...
					else
					{
						...
						itemRange.end = (max == 6 && itemRange.end == 7) ? 0 : itemRange.end;
					}
				}
			}
